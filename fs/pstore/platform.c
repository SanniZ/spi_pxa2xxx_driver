			...
			enum kmsg_dump_reason reason)
{
	...
	while (total < kmsg_bytes) {
		...
		ret = psinfo->write(PSTORE_TYPE_DMESG, reason, &id, part,
				    oopscount, compressed, total_len, NULL,
				    psinfo);
	...
	if (spin_trylock_irqsave(&pstore_extra_dumper_lock, flags_extra)) {
		...
		{
			/* Dump each extra dumper from its first part */
			part = 1;
			...
			while (pstore_extra_written < pstore_extra_size) {
				...
				ret = psinfo->write(PSTORE_TYPE_EXTRA, reason, &id, part,
						oopscount, false, read,
						extra_dumper->name, psinfo);
...
static void pstore_console_write(struct console *con, const char *s, unsigned c)
{
	...
	while (s < e) {
		...
		psinfo->write(PSTORE_TYPE_CONSOLE, 0, &id, 0, 0, 0, c, NULL,
			      psinfo);
		spin_unlock_irqrestore(&psinfo->buf_lock, flags);
		s += c;
		c = e - s;
...
static int pstore_write_compat(enum pstore_type_id type,
			       ...
			       const char *tag, struct pstore_info *psi)
{
...
void pstore_get_records(int quiet)
{
	...
	while ((size = psi->read(&id, &type, &count, &time, &buf, &compressed,
		...
		rc = pstore_mkfile(type, psi->name, tag, id, count, buf,
