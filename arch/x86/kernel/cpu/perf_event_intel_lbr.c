...
enum {
	...
	LBR_FORMAT_LIP_FLAGS3	= 0x06,
	LBR_FORMAT_MAX_KNOWN    = LBR_FORMAT_LIP_FLAGS3,
};
...
static enum {
	...
	[LBR_FORMAT_LIP_FLAGS3] = LBR_EIP_FLAGS | LBR_IPDELTA | LBR_TIME
};
...
void intel_pmu_lbr_read_64(struct cpu_hw_events *cpuc)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (lbr_flags & LBR_IPDELTA) {
			ipdelta = (from << (64 - 51)) >> (48 + (64 - 51));
			skip = (64 - 48);
		}
		from = (u64)((((s64)from) << skip) >> skip);

		skip = 0;
		if (lbr_flags & LBR_TIME) {
			time = to >> 48;
			skip = (64 - 48);
		}
		to = (u64)((((s64)to) << skip) >> skip);
		...
		cpuc->lbr_entries[out].ipdelta	 = ipdelta;
		cpuc->lbr_entries[out].cycles	 = time;
