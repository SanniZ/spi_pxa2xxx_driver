...
int xhci_reset(struct xhci_hcd *xhci)
{
	...
	if (xhci->quirks & XHCI_HCD_RST_GET_STUCK)
		intel_bxt_hcd_reset_quirk(
		to_pci_dev(xhci_to_hcd(xhci)->self.controller), 0);
	...
	if (xhci->quirks & XHCI_HCD_RST_GET_STUCK)
		intel_bxt_hcd_reset_quirk(
		to_pci_dev(xhci_to_hcd(xhci)->self.controller), 1);
