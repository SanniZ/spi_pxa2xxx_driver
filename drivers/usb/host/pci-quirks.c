...
#define USB_INTEL_PCE		   0xA2
#define USB_INTEL_PCE_HAE	   (1 << 5)
...
/* This is one WA for xHCI of Intel BXT platform.
 * If driver issues HCReset and one or more port initiates Receiver detection
 * right away, port may not transition into Warm Reset State for HCReset.
 * During this time if D0I2 timer expires, controller may enter D0I2 and it
 * may get stuck there. So this will be a hang scenario for the controller.
 *
 * The WA is disable d0i2 before trigger hcd reset and re-enable it after done.
 **/
void intel_bxt_hcd_reset_quirk(struct pci_dev *xhci_pdev, int post)
{
	static u32 pce;

	pci_read_config_dword(xhci_pdev, USB_INTEL_PCE,
			&pce);

	if (post)
		pce |= USB_INTEL_PCE_HAE;
	else
		pce &= ~USB_INTEL_PCE_HAE;

	pci_write_config_dword(xhci_pdev, USB_INTEL_PCE,
			pce);
}
