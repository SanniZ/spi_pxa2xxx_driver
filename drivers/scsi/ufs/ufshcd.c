				  ...
				  u32 param_size)
{
	...
	if (ret || (buff_len > ufs_query_desc_max_size[desc_id])
	    || (desc_buf[QUERY_DESC_LENGTH_OFFSET] != buff_len)
...
static inline int ufshcd_read_device_desc_param(struct ufs_hba *hba,
						u32 param_offset,
						u8 *buf,
						u32 size)
{
	return ufshcd_read_desc_param(hba, QUERY_DESC_IDN_DEVICE, 0,
					param_offset, buf, size);
}
...
static int ufshcd_read_serial_no(struct ufs_hba *hba,
				 const u8 **serial_no, size_t *serial_no_len)
{
	struct uc_string_id *uc;
	char *no;
	u8 sn_no_idx;
	u8 uc_len;
	int ret;
	...
	ret = ufshcd_read_device_desc_param(hba, DEVICE_DESC_PARAM_SN,
					    &sn_no_idx, sizeof(sn_no_idx));

	if (ret) {
		dev_err(hba->dev, "Failed reading device sn no idx ret = %d\n",
			ret);
		goto out;
	}

	ret = ufshcd_read_desc(hba, QUERY_DESC_IDN_STRING,
			       sn_no_idx, (u8 *)uc, QUERY_DESC_STRING_MAX_SIZE);
	if (ret) {
		dev_err(hba->dev, "Failed reading sn no id ret = %d\n", ret);
		goto out;
	}
	...
	if (uc->len < 2) {
		*serial_no_len = 0;
		*serial_no = NULL;
		dev_warn(hba->dev, "Serial No has length < 2\n");
		goto out;
	}
	uc_len = uc->len - 2;

	print_hex_dump_debug("ufs serial no: ", DUMP_PREFIX_OFFSET,
			     16, 1, uc->uc, uc_len, true);

	no = kmalloc(uc_len, GFP_KERNEL);
	if (!no) {
		ret = -ENOMEM;
		goto out;
	}
	memcpy(no, uc->uc, uc_len);

	*serial_no = no;
	*serial_no_len = uc_len;
...
static inline void ufshcd_rpmb_add(struct ufs_hba *hba)
{
	...
	if (ret) {
		...
		/* fallback to single frame write */
		rw_size = 1;
	}

	ufshcd_rpmb_dev_ops.reliable_wr_cnt = rw_size;

	ret = ufshcd_read_serial_no(hba, &ufshcd_rpmb_dev_ops.dev_id,
				    &ufshcd_rpmb_dev_ops.dev_id_len);
