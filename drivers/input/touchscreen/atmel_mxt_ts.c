				...
				const struct firmware *fw)
{
	...
	if (pos < 0) {
		dev_err(&data->client->dev,
			"Invalid firmware, too short (%zu bytes, want %d)\n",
			fw->size, MXT_FW_INFO_SIZE);
		return true;
	}
...
static int mxt_input_open(struct input_dev *dev)
{
	...
	mutex_lock(&data->pm_sysfs_lock);
	mxt_start(data);
	data->opened = true;
	mutex_unlock(&data->pm_sysfs_lock);
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	ret = copy_from_user((void*)cover, buf,
			     sizeof(buf) > 16 ? 16 : sizeof(buf));
	if (ret || cover[0] != '1')
		return -EFAULT;
		...
		const struct i2c_device_id *id)
{
	...
	if (!data->pdata) {
		...
		gpio = devm_gpiod_get_index(&client->dev, "atml_gpio_rst",
					    0, GPIOD_ASIS);
		if (!IS_ERR(gpio)) {
		...
		gpio = devm_gpiod_get_index(&client->dev, "atml_gpio_switch",
					    1, GPIOD_ASIS);
		if (!IS_ERR(gpio)) {
		...
		gpio = devm_gpiod_get_index(&client->dev, "atml_gpio_int",
					    2, GPIOD_ASIS);
		if (!IS_ERR(gpio)) {
...
static int mxt_resume(struct device *dev)
{
	...
	/*
	 * In Atmel mXT1668T2 Revision 1.0 documentation page 35, Section 7 :
	 * "After power-up, the device takes 86ms before it is ready
