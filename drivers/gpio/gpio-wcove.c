...
static int wcove_gpio_dir_in(struct gpio_chip *chip, unsigned gpio)
{
...
static int wcove_gpio_dir_out(struct gpio_chip *chip, unsigned gpio,
				    int value)
{
	struct wcove_gpio *wg = to_wg(chip);
...
static int wcove_gpio_get(struct gpio_chip *chip, unsigned gpio)
{
	struct wcove_gpio *wg = to_wg(chip);
	int ret;
	unsigned int val;
...
static int wcove_irq_type(struct irq_data *data, unsigned type)
{
