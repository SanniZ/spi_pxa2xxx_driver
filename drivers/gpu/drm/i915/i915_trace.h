...
TRACE_EVENT(i915_tdr_hang_check,
	    TP_PROTO(struct intel_engine_cs *engine,
		     ...
		     u64 acthd,
		     bool busy),
	    TP_ARGS(engine, seqno, hd, acthd, busy),
	    ...
	    TP_printk("dev=%u, engine=%s, seqno=%u (%d), last seqno=%u (%d),"
		      " head=0x%x, acthd=0x%lx,"
		      " score=%d%s, action=%u [%s], busy=%s",
...
/**
 * DOC: i915_error_recovery_queued
 *
 ...
 * it will be set to "none". If engine reset fails or not available then we
 * fallback to full GPU reset, which is remarked by appending the
 * "[FALLBACK]" flag.
 */
TRACE_EVENT(i915_error_recovery_queued,
	TP_PROTO(struct drm_device *dev,
		 u32 engine_mask,
		 bool reset_in_progress),

	    TP_ARGS(dev, engine_mask, reset_in_progress),

	TP_STRUCT__entry(
			__field(u32, dev)
			__field(u32, engine_mask)
			__field(bool, reset_in_progress)
	),

	TP_fast_assign(
			__entry->dev = dev->primary->index;
			__entry->engine_mask = engine_mask;
			__entry->reset_in_progress = reset_in_progress;
	),

	TP_printk("dev=%u, engine_mask=0x%02x%s%s%s%s%s%s%s, reset_in_progress=%s",
		  __entry->dev,
		  __entry->engine_mask,
		  __entry->engine_mask ? " (":"",
		  __entry->engine_mask & RENDER_RING ? " [RCS] " : "",
		  __entry->engine_mask & BSD_RING ? 	" [VCS] " : "",
		  __entry->engine_mask & BLT_RING ? 	" [BCS] " : "",
		  __entry->engine_mask & VEBOX_RING ? 	" [VECS] " : "",
		  __entry->engine_mask & BSD2_RING ? 	" [VCS2] " : "",
		  __entry->engine_mask ? ")":"",
		  __entry->reset_in_progress ?
			(__entry->engine_mask ? "true [FALLBACK]" : "true") :
				"false")
);

/**
 * DOC: i915_engine_recovery_begin
 *
 * This tracepoint tracks hang recovery start
 */
TRACE_EVENT(i915_engine_recovery_begin,
	TP_PROTO(struct intel_engine_cs *engine),
	...
	TP_printk("%s, hang recovery started", __entry->engine->name)
);

/**
 * DOC: i915_engine_recovery_complete
 *
 * This tracepoint tracks hang recovery completion
 */
TRACE_EVENT(i915_engine_recovery_complete,
	TP_PROTO(struct intel_engine_cs *engine),
	...
	TP_printk("%s, hang recovery complete", __entry->engine->name)
);
