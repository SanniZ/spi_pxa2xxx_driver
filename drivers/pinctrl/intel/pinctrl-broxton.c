...
/* UART2 RXD and UART3 CTS may be used as wake-up pins */
static const unsigned bxt_north_uart2_rxd_pins[] = { 46 };
static const unsigned bxt_north_uart3_cts_pins[] = { 61 };
...
static const struct intel_pingroup bxt_north_groups[] = {
	...
	/*
	*  Change the UART2 RXD and UART3 CTS to gpio mode
	*  before device enter d0i3, change back to UART mode
	*  after device wake up to d0.
	*/
	PIN_GROUP("uart2_rxd_d0i3_grp", bxt_north_uart2_rxd_pins, 0),
	PIN_GROUP("uart2_rxd_d0_grp", bxt_north_uart2_rxd_pins, 1),
	PIN_GROUP("uart3_cts_d0i3_grp", bxt_north_uart3_cts_pins, 0),
	PIN_GROUP("uart3_cts_d0_grp", bxt_north_uart3_cts_pins, 3),
};
...
static const char * const bxt_north_uart2_rxd_groups[] = {
	"uart2_rxd_d0i3_grp", "uart2_rxd_d0_grp",
};
static const char * const bxt_north_uart3_cts_groups[] = {
	"uart3_cts_d0i3_grp", "uart3_cts_d0_grp",
};
...
static const struct intel_function bxt_north_functions[] = {
	...
	FUNCTION("uart2_rxd", bxt_north_uart2_rxd_groups),
	FUNCTION("uart3_cts", bxt_north_uart3_cts_groups),
};
...
/*******************************************************
* Below is the pinctrl map for UART GPIO Wake-up
* ******************************************************/

/*
* For X86 platforms, Currently there is no way to put the pinctrl_map to
* device tree. Aslo, no board file to put this pinctrl_map.
* Before the ACPI based pinctrl_map support completed, put the pinctrl_map
* here for other drivers to use pinctrl functions.
*/

static struct pinctrl_map bxt_north_pinmap[] = {
	PIN_MAP_MUX_GROUP_DEFAULT("0000:00:18.2", "INT34D1:00",
		"uart2_rxd_d0_grp", "uart2_rxd"),
	PIN_MAP_MUX_GROUP("0000:00:18.2", PINCTRL_STATE_SLEEP, "INT34D1:00",
		"uart2_rxd_d0i3_grp", "uart2_rxd"),

	PIN_MAP_MUX_GROUP_DEFAULT("0000:00:18.3", "INT34D1:00",
		"uart3_cts_d0_grp", "uart3_cts"),
	PIN_MAP_MUX_GROUP("0000:00:18.3", PINCTRL_STATE_SLEEP, "INT34D1:00",
		"uart3_cts_d0i3_grp", "uart3_cts"),
};
...
static const struct intel_pinctrl_soc_data bxt_north_soc_data = {
	...
	.maps = bxt_north_pinmap,
	.nmaps = ARRAY_SIZE(bxt_north_pinmap),
};
...
/*******************************************************
* Below is the pinctrl map for BXT south
* ******************************************************/

/*
* Toggle RX inversion bit on GPIO_200 (DBI_RESX) to manually trigger
* HPD interrupt.
*/

static unsigned long no_rx_inv_mode[] = {
	PIN_CONF_PACKED(PIN_CONF_INTEL_INV_RX, 0),
};

static unsigned long rx_inv_mode[] = {
	PIN_CONF_PACKED(PIN_CONF_INTEL_INV_RX, 1),
};

static struct pinctrl_map bxt_south_pinmap[] = {
    PIN_MAP_CONFIGS_PIN("i2c-PI330532:00", PINCTRL_STATE_DEFAULT,
		"INT34D1:04", "DBI_RESX", no_rx_inv_mode),

	PIN_MAP_CONFIGS_PIN("i2c-PI330532:00", PINCTRL_STATE_SLEEP,
		"INT34D1:04", "DBI_RESX", rx_inv_mode),
};
...
static const struct intel_pinctrl_soc_data bxt_south_soc_data = {
	...
	.maps = bxt_south_pinmap,
	.nmaps = ARRAY_SIZE(bxt_south_pinmap),
};
...
static int bxt_pinctrl_probe(struct platform_device *pdev)
{
	...
	ret = intel_pinctrl_probe(pdev, soc_data);
	if (!ret) {
		/* register pinctrl map */
		if (soc_data->maps)
			ret = pinctrl_register_mappings(soc_data->maps, soc_data->nmaps);
	}
